...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  LoginBloc({required FirebaseAuthenticationRepository authenticationRepository})
  ...
  /// Tries sign-up; if email already in use, falls back to sign-in.
  Future<void> _onContinueWithEmail(
      ContinueWithEmailAndPassword event, Emitter<LoginState> emit) async {
    emit(state.copyWith(status: FormzSubmissionStatus.inProgress));
    try {
      await _authenticationRepository.signUpWithEmailAndPassword(
          email: event.email, password: event.password);
